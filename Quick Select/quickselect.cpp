...
/**
 * Partitons subarray by Lomuto's algorithm using first element as pivot
 */
...
int main(int argc, char *argv[]) {
    ...
    if(k>num_values && num_values==1){
    	cerr<<"Error: Cannot find smallest element " << argv[1] << " with only 1 value."<<endl;
    	return 1;
    }
    if(k>num_values){
	...
	int answer=quick_select(values.data(),values.size(),k);
