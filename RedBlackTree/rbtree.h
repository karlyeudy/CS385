/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Tarquin Bennett and Anna Brown
 * Version     : 1.0
...
private:
    ...
    int diameter(Node<K, V> *node) const {
    	if (node == nullptr)
    	     return 0;
    	int sum_height = (height(node->left)+height(node->right)+2);
    	int max_diam = std::max(diameter(node->left),diameter(node->right));
    	return std::max(sum_height,max_diam);
    }
